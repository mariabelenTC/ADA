...
--------------------------------------------------------------------------------
	...
	exception
		when others =>
			ATIO.Put_Line("Usage:  ./chat_server_2  <Puerto> <Numero de clientes>");
			ATIO.Put_Line("o");
			ATIO.Put_Line("Usage:  ./chat_client_2  <Maquina>  <Puerto>  <Nickname> ");
...
--____________________________________________________________________________--
	...
	begin
		...
		if Type_Mess = CM.Server then
			ATIO.New_Line;
			Extract_Message_Server(P_Buffer,name,Text);
...
--____________________________________________________________________________--
	...
	begin
		...
		while Active_Clients.Has_Element(C) loop
			Build_Menssage_Banned(P_Buffer,Old_Nick);
			Client_EP:=Active_Clients.Element(C).Value.EP;
			LLU.Send(Client_EP,P_Buffer);
			Active_Clients.Next(C);
		end loop;
