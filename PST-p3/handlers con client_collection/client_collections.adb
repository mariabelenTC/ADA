...
--------------------------------------------------------------------------------
	function Collection_Image(Collection: in Collection_Type)return String is
		P_Aux:Cell_A;
		Usuario,Usuarios:ASU.Unbounded_String;


	begin
		P_Aux:=Collection.P_First;
		if P_Aux=null then
			Usuarios:= ASU.To_Unbounded_String("");
		else
			Usuario:=ASU.To_Unbounded_String(print_EP_y_Nick(P_Aux)
										& ASCII.LF & print_EP_y_Nick(P_Aux.Next));
			P_Aux:=P_Aux.Next;
		end if;

		while P_Aux /= null loop

			Usuarios:=ASU.To_Unbounded_String(ASU.To_String(Usuario)
									& ASCII.LF & (print_EP_y_Nick(P_Aux)
									& print_EP_y_Nick(P_Aux.Next)));
		   P_Aux:=P_Aux.Next;
		end loop;
